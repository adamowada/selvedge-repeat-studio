...
- Drag to move, use corner handles to resize uniformly, or use the rotation handle. Scale (%) and Rotation (°) in the Inspector provide keyboard alternatives: Enter/blur commits; Escape cancels. Flip, duplicate, delete and stacking controls act on the selected placement. Drag rows in Placements to reorder layers (topmost first), or focus a row and press Alt + Up/Down. Each reorder is one undo step; dropping outside the list cancels.
