...
with sync_playwright() as p:
    ...
    check('all source/placement thumbnails disable native dragging',page.locator('.asset-sidebar img').evaluate_all('(images)=>images.every(i=>i.draggable===false)'))
    ...
    browser.close()
(project/'verification/v4/style-state.json').write_text(json.dumps({'scope':'Native DOM/CSS on static source fixtures; NOT React/Konva runtime verification. Not a full accessibility audit.','results':results},indent=2))
