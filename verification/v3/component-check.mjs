...
check('color updates locally and commits on blur',()=>{
  const seen=[];const c=controls(value=>{seen.push(value);return true;});
  byLabel(c.render(),'Background color').props.onChange({target:{value:'#345678'}});
  assert.equal(seen.length,0);
  byLabel(c.render(),'Background color').props.onBlur();
...
check('transparent toggles the existing background contract without losing placement transforms',()=>{
  ...
  assert.equal(saved.background,null);assert.equal(saved.placements,doc.placements);
});
