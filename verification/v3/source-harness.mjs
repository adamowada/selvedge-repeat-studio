...
const attrs = props => Object.entries(props).filter(([k,v]) => !['children','key','ref','style'].includes(k) && !k.startsWith('on') && v != null && (v !== false || k.startsWith('aria-')) && typeof v !== 'object').map(([k,v]) => ` ${k === 'className' ? 'class' : k === 'tabIndex' ? 'tabindex' : k === 'strokeWidth' ? 'stroke-width' : k === 'strokeLinecap' ? 'stroke-linecap' : k === 'strokeLinejoin' ? 'stroke-linejoin' : k}="${escape(typeof v === 'boolean' && k.startsWith('aria-') ? String(v) : v === true ? '' : v)}"`).join('');
...
export function makeFixture(kind, viewWidth = 1480, viewHeight = 900) {
  ...
  for (const key of ['action','addDemo','tryDocument','setImportErrors','onFiles','insert','selectPlacement','setSelection','begin','onNode','end','nudge','endNudge','fit','setError','onStage','onSize','tryCamera','doExport']) out[key]=noop;
...
if (process.argv[1] && import.meta.url === pathToFileURL(process.argv[1]).href) {
  const destination = process.argv[2] || join(project,'verification/v3/static-fixtures');
